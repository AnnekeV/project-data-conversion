...
Made an change in read.py to remove depSM from the column of depth
  File "/Users/annek/opt/anaconda3/envs/nioz_ctd/lib/python3.9/site-packages/ctd/read.py", line 396, in from_cnv
...
labels = {'temp':'Potential temperature [C]', 'sal':'Salinity [PSU]', 'month':'Month', 'depth':"Depth [m]", '2018.0':"",'2019.0':"",}
...
Y = cast_diffusion["dens"].values.reshape(-1, 1)  # -1 means that calculate the dimension of rows, but have 1 column
...
cast_540['spring_neap'] = cast_540['tidal_amp'].rolling(center=True, window = 6*25).max()
...
if mooring == "GF10":
    # GF10 microcat files
    files = ["20190612_SBE5968_540m.cnv","20190612_SBE5969_330m.cnv", "20190612_SBE5970_150m.cnv", "20190612_SBE5971_60m.cnv", "20190821_SBE4247_5m.cnv","20180828_SBE2989_5m.cnv"]
    files = files[::-1]
    fpath = fpath_GF10
elif mooring == "GF13":
    # GF13 microcat files
    files = ["20190802_SBE16523_20m.cnv","20190802_SBE15390_60m.cnv","20190802_SBE15399_120m.cnv"]
    fpath = fpath_GF13
else: files = ""
...
for i in range(nr_moorings):
    ...
    if files[i] == "20180828_SBE2989_5m.cnv": start_time = "Jan 1 2017" 
    else: start_time = "Jan 1 2018" 
    ...
    for j in range(len(variables)):
        ...
        cast.plot.line(x='date',y=variables[j], ax=ax, 
                    # title = title, 
                    legend=True,
                    label = title,
                    # s=.5
...
for i in range(nr_moorings):
    cast, metadata,cast = open_cnv(fpath + files[i])
    time_to_date(fpath + files[i], cast, start_time)
    ax = cast.groupby(cast.date.dt.date).mean()['Pressure [dbar]'].apply(
        lambda x: x-cast['Pressure [dbar]'].mean()).iloc[:-1].plot(label=files[i])
ax.xaxis.set_major_formatter(
    dates.ConciseDateFormatter(ax.xaxis.get_major_locator()))
plt.legend()
...
for i in range(nr_moorings):
    cast, metadata,cast = open_cnv(fpath + files[i])
    time_to_date(fpath + files[i], cast, start_time)
    ax2 = cast['Pressure [dbar]'].apply(
        lambda x: x-cast['Pressure [dbar]'].mean()).iloc[:-1].plot(label=files[i].split("_")[2].split(".")[0])
    max_tide = cast['Pressure [dbar]'].apply( lambda x: x-cast['Pressure [dbar]'].mean()).abs().max()
...
for i in range(nr_moorings):
    ...
    # )
    cast.rolling(6*24*7).mean().plot(x='sal',y='temp', ax=ax, 
                legend=True,
                label =  title,
    )
...
df_monthly = pd.read_csv(Path.cwd().parent.joinpath("intermediate_files", "monthly_18_19_gf10.csv"), index_col=0, parse_dates=True)
...
if nr_moorings == 6: nr_moorings-=1
...
for i in range(3):
    for j in range(nr_moorings):
        ...
        var = vari[i]
        ax = axes[j,i]
        month_mask = (df_monthly["Pressure [dbar]"] >d-1) & (df_monthly["Pressure [dbar]"]<d+1)
        all_moorings[all_moorings.id == str(d)].plot.scatter(x='date' , y=var, ax=ax, color ='red', s=0.5, label='mooring')
        df_monthly[month_mask].plot(x='date' , y=var, ax=ax, color='darkred', label='monthly')
        df_monthly[month_mask].plot.scatter(x='date' , y=var, ax=ax, color='black', label=None)
...
# %%
df_monthly[df_monthly["Pressure [dbar]"]== min(df_monthly["Pressure [dbar]"], key= lambda x: abs(x-d))]
...
maski = df[mask][df[mask].rolling(3, center=True).max().Pxx_den == df[mask].Pxx_den].index

fig = go.Figure()
df.P = df.P/24
fig.add_trace(go.Scatter(x=df.P, y=df.Pxx_den, fill='tozeroy', 
                    mode= 'none',
                    ))
dfi = df.loc[maski]
fig.add_trace(go.Scatter(x=dfi["P"], y=dfi["Pxx_den"], 
                    text=dfi["P"].round(1),
...
fig.update_layout(title=var_freq, showlegend=False, 
...
all_moorings['month'] = all_moorings.date.apply(lambda x: x.strftime("%B") + " ") + all_moorings['year'].astype(str)
...
# mask = :
fig = all_moorings.sort_values([ "date"]).groupby(['depth', 'month', 'day'],as_index=False, sort=False).mean().plot.scatter(x='sal',y='temp', 
            # ax=ax, 
            color = 'month',
            symbol='year',
            # legend=True,
            # label =  title,
            labels=labels,
            facet_col = "depth", facet_col_wrap =3,
...
# %%
df_TS = all_moorings.sort_values([ "date"]).groupby(['depth','month', 'day'],as_index=False, sort=False).mean()
...
for i in range(nr_moorings):
    ...
    time_to_date(fpath + files[i], cast, start_time)
    cast = cast.set_index('date').resample(resample_interval).mean().reset_index().round({"Pressure [dbar]":2})
    cast["depth"] = cast["Pressure [dbar]"].mean()
    all_moorings_hourly = pd.concat([all_moorings_hourly, cast]).reset_index(drop=True)


all_moorings_hourly.to_csv(f"{path_parent}/Processing/intermediate_files/mooring_gf10_{resample_interval}hourly.csv")
...
df19['time'] =(df19.time.apply(lambda x: timedelta(days=x))+timedelta(days=365))/ np.timedelta64(1, 'D')
...
df_short = df_mi.reset_index().set_index('date')["2018-01-01":"2020-01-01"].reset_index().set_index(["Pressure [dbar]", "time"])
...
y_depths = df_short.reset_index().depth.fillna(long_py)   #data series with all the depths with mounting depth for 
...
X,Y= np.meshgrid(np.arange(round(min(long_px)),max(long_px),1),np.arange(2,553,1))
...
df_interpolated.to_csv(path_parent.joinpath("Processing", "intermediate_files","interpolated_mooring_profile_2018_2019", f"{var}.csv"))
...
ax2.scatter(x_dates.dt.floor('D'), y_depths, c="white", s=.1,alpha =.5)
...
def dfprofile_to_array(df, direction):
    if direction == "H":
        return [df.date.apply(lambda x: x.to_julian_date()).values, df[var].values, np.mean(df["Pressure [dbar]"].values), ]
    elif direction == "V":
        return [df["Pressure [dbar]"].values, df[var].values, np.mean(df.date.apply(lambda x: x.to_julian_date()).values) ]
    else: print ("error")

print(var)
maskV1 =( df_combi_hr.Type =="CTD")& ( df_combi_hr.date> datetime(2018,6,1))& ( df_combi_hr.date< datetime(2018,6,15)) & (df_combi_hr["Pressure [dbar]"] <60)  & (df_combi_hr["Pressure [dbar]"] > 5)
maskV2 = mask =( df_combi_hr.Type =="CTD")& ( df_combi_hr.date> datetime(2018,6,15))& ( df_combi_hr.date< datetime(2018,7,1))  & (df_combi_hr["Pressure [dbar]"] <60)  & (df_combi_hr["Pressure [dbar]"] > 5)
...
maskH1 = (df_combi_hr.Type =="Mooring") & (df_combi_hr.date>V1.date.max()) &   (df_combi_hr.date<V2.date.min()) & (df_combi_hr.depth <10)
maskH2 = (df_combi_hr.Type =="Mooring") & (df_combi_hr.date>V1.date.max()) &   (df_combi_hr.date<V2.date.min()) & (df_combi_hr.depth >10) & (df_combi_hr.depth <100)
...
TargetWJB =[np.arange(6,60,3),  np.arange(V1.date.max().to_julian_date(), V2.date.min().to_julian_date(),1./24.)]

# from WJB_profile_interpolate_v2 import WJB_profile_interpolate

Q,  [QV1B, QV1T, QV1], [QHD1, QHD2, QHD]  = WJB_profile_interpolate(V1profile, V2profile,  H2profile, H1profile, TargetWJB, 'LinearV')
#  WJB_profile_interpolate(Vprofile1, Vprofile2, HserieUp, HserieDown, Target, Method)

# %%


scat = plt.scatter(np.repeat(V1profile[2], len(V1profile[0])), V1profile[0], c=V1profile[1])
