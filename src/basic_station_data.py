...
path_stations = path_parent.joinpath('Data', 'Stations GHF.csv')
...
def find_closest_stations(pointA):
    ...
    for i in range(len(stat_loc.index)):
        dist_st =  dist_lat_lon((stat_loc.loc[i,'Latitude'], stat_loc.loc[i,'Longitude']), pointA )  # distance to station
...
def find_distance_from_fjordmouth(pointA):
    ...
    dist_between = station_index.loc[closest["name"], "Distance"] - station_index.loc[second["name"], "Distance"] # distance between stations
    if closest["distance"] < 1.:
        ...
        dist_from_mouth = 999
        print(f"Something is wrong with {pointA}, distance to closest station is {closest['distance']:.1f} km")
    else: 
        if dist_between <0:  # if the closest station is closer to the mouth of the fjord
            dist_from_mouth = station_index.loc[closest["name"], "Distance"] + closest['distance'] 
