...
class InteractivePlot:
    ...
    def on_key_press(self, event):
        ...
        if event.key == 'y':
            print("Yes")
            plt.close(self.fig)
            CTD_monthly.save_dsCTD_to_netcdf(ds_single_CTD, CTD_monthly.path_intermediate_files_netcdf)
...
if __name__ == "__main__":
    ...
    for year in ["2018", "2019"]:
        path_data, stat = CTD_monthly.extract_station_info(CTD_monthly.path_parent, year)
        ...
        for fname in Path(path_data).rglob("*.cnv"):
            ...
            ds_single_CTD = CTD_monthly.make_xarray_with_attributes(down, metadata, this_stat)
