...
for year in ['2018', '2019']:
    ...
    stat = pd.read_fwf(rf"{station_info}", header = 1,  encoding='latin1', on_bad_lines='skip', widths=widths, skipfooter=4)
    stat['Latitude'] = stat['Latitude'].apply(lambda x: float(x.split(" ")[0])+ float(x.split(" ")[1])/60 )
    stat['Longitude'] = stat['Longitude'].apply(lambda x: -1*(float(x.split(" ")[0])+ float(x.split(" ")[1])/60 ))
    stat['Coordinates'] = stat.apply(lambda row: Point(latitude=row['Latitude'], longitude=row['Longitude']), axis=1)
    stat["Date"] = pd.to_datetime(stat['YYYYMMDD'], format='%Y%m%d')
    stat["Name"] = stat['St.No.'].copy()
    stat = stat.set_index(stat['St.No.'])
    stat["Distance"] = stat.merge(stat_loc.rename(columns={'Station':'St.'}), on='St.', how='left')["LengthMouth"].values
    nan_dist = stat[stat["Distance"].isna()].index
    for i in nan_dist:
        p = ((stat.loc[i,'Latitude']),(stat.loc[i,'Longitude']))
        dist = find_distance_from_fjordmouth(p)
        if dist == 999: print(i)
        else: stat.loc[i,'Distance'] = dist

    stat[['YYYYMMDD', 'St.']].value_counts().reset_index().pivot(index='YYYYMMDD',columns='St.').to_csv(f"{path_data}/overview_per_date.csv")
    ...
    counter=0
    plot_cast = False
    fileNames = [f for f in listdir(path_data) if os.path.isfile(os.path.join(path_data, f))]
    ...
    for fname in Path(path_data).rglob('*.cnv'):
        ...
        down, up = cast.split()
        down = down.rename(columns={'potemp090C':'Potential temperature [°C]', 'sal00':'Salinity [PSU]', 'density00':'Density [kg/m3]'})
        up = up.rename(columns={'potemp090C':'Potential temperature [°C]', 'sal00':'Salinity [PSU]', 'density00':'Density [kg/m3]'})
        ...
        meta = ['Latitude', 'Longitude', 'St.','Date', 'Type', 'Name', "YYYYMMDD", "Distance"]
        for key in meta:
            down[key]=this_stat[key]
        
        if plot_cast == True:
            fig, (ax1, ax2, ax3) = plt.subplots(1, 3, sharey=True)
            down.reset_index().plot(x='Potential temperature [°C]', y='Pressure [dbar]', ax=ax1, legend=False).invert_yaxis()
            down.reset_index().plot(x='Salinity [PSU]', y='Pressure [dbar]', ax=ax2, legend=False)
            down.reset_index().plot(x='Density [kg/m3]', y='Pressure [dbar]', ax=ax3, legend=False)
            fig.suptitle(f"{metadata['name']}, {this_stat['St.']}, {this_stat['Date']}, {this_stat['Type']}")
    ...
    GF10 = all_thisyear[all_thisyear['St.']== 'GF10'].sort_values(["Name","Pressure [dbar]"]).reset_index(drop=True)
    ...
    column_rename = {'potemp090C':'temp', 'sal00':'sal', 
                            'density00':'dens', 'timeJV2':'time','timeJ':'time', 
                            'sigma-t00':"sigma-dens", 'sigma-È00':"sigma-dens", 
                            "sigma-�00":"sigma-dens",
                            'Date':'date', 'Potential temperature [°C]':'temp',
                            'Salinity [PSU]':'sal', 'Density [kg/m3]':'dens',
                            }
    df10.rename(columns=column_rename).reset_index().to_csv(f"{path_intermediate_files}/monthly_{year}_gf10.csv")
...
all_years.to_csv(f"{path_intermediate_files}/monthly_all_years_all_stations.csv", index=None)
...
df_monthly.date = df_monthly.date.apply(lambda x: datetime.strptime(x, '%Y-%m-%d'), 'broadcast')
...
fnames = []
for fname in Path(os.path.join(path_parent, "Data","CTD")).rglob('*.cnv'): fnames.append(str(fname))
...
g_simple = dSet['Potential temperature [°C]'].plot.contourf(x="Longitude", y="Pressure [dbar]", col="Date", col_wrap=4, 
                                                   yincrease=False, cmap=plt.get_cmap('inferno'), levels=np.arange(-2,4,.2))
    
g_simple = dSet['Salinity [PSU]'].plot(x="Longitude", y="Pressure [dbar]", col="Date", col_wrap=4, 
...
april = all_thisyear[all_thisyear.Date ==Dates[0]].sort_values(['Pressure [dbar]', 'Longitude'])
...
for i, ax in enumerate(axes.flat):
    if i == len(StGF10): break
    down = all_thisyear[all_thisyear['Name'] == StGF10[i]]
    if i == 0: 
        down.reset_index().plot(x='Salinity [PSU]', y='Pressure [dbar]', ax=ax, legend=False, title= down['YYYYMMDD'][0]).invert_yaxis()
        ax.set_ylabel('Pressure [dbar]')

    else:
        down.reset_index().plot(x='Salinity [PSU]', y='Pressure [dbar]', ax=ax, legend=False, title= down['YYYYMMDD'][0])
    if i%5==0:
        ax.set_ylabel('Pressure [dbar]')
...
for i, ax in enumerate(axes.flat):
    ...
    down = all_thisyear[all_thisyear['Name'] == StGF10[i]]
    if i == 0: 
        down.reset_index().plot(x='Potential temperature [°C]', y='Pressure [dbar]', ax=ax, legend=False, title= down['YYYYMMDD'][0]).invert_yaxis()
        ax.set_ylabel('Pressure [dbar]')

    else:
        down.reset_index().plot(x='Potential temperature [°C]', y='Pressure [dbar]', ax=ax, legend=False, title= down['YYYYMMDD'][0])
    if i%5==0:
        ax.set_ylabel('Pressure [dbar]')
    ax.set_xlim([-1.5,3])
# fig.suptitle('Salinity in GF10')
...
for i, ax in enumerate(axes.flat):
    if i == len(StGF10): break
    down = all_thisyear[all_thisyear['Name'] == StGF10[i]]
    down.reset_index().plot(y='Potential temperature [°C]', x='Salinity [PSU]', ax=ax, legend=False, title= down['YYYYMMDD'][0])
    if i%5==0:
...
for i in range(len(StGF10)):
    down = all_thisyear[all_thisyear['Name'] == StGF10[i]]
    down.reset_index().plot(y='Potential temperature [°C]', x='Salinity [PSU]' , ax=ax,label= down['YYYYMMDD'][0])
ax.set_xlim([30.5,34])
# fig.suptitle('Salinity in GF10')
plt.tight_layout()
...
# Mooring year
df_monthly = pd.read_csv(f"{path_intermediate_files}/monthly_18_19_gf10.csv", index_col=0, parse_dates=["date"])
df_gf10 = df_monthly[(df_monthly['St.']=="GF10")&(df_monthly['Type']=='CTD')]
df10piv = df_gf10.drop_duplicates(['date', 'Pressure [dbar]']).pivot('date', 'Pressure [dbar]',var)['2018-05-29':'2019-06-12']
...
df_gf10 = df_monthly[(df_monthly['St.']=="GF10")&(df_monthly['Type']=='CTD')]
df10piv = df_gf10.drop_duplicates(['date', 'Pressure [dbar]']).pivot('date', 'Pressure [dbar]','temp')['2018-05-29':'2019-06-12']
...
plt.ylim([500,0])
fig.colorbar(contourf, orientation='vertical', label=variable, ticks=levels_f[::2], )
...
df_both = pd.read_csv(f"{path_parent}/Processing/intermediate_files/monthly_18_19_gf10.csv", index_col=0)
names = df_both.Name.unique()

bin = 20 #m
df_both["Pressure_grouped"] = df_both["Pressure [dbar]"].apply(lambda x: x-np.mod(x,bin))
...
df_fresh = df_both.groupby(["date", "Layers"]).agg({"rel_sal":'sum', "sal":"mean","temp":"mean"}).reset_index()

pd.options.plotting.backend = "plotly"

df_fresh_profile = df_both.groupby(["date", "Pressure_grouped"]).agg({"rel_sal":'mean', "sal":"mean","temp":"mean"}).reset_index()
df_fresh_profile.rel_sal *=bin

fig = df_fresh_profile.plot.bar(
    x="rel_sal", y="Pressure_grouped", 
    facet_col="date", facet_col_wrap=9, 
    orientation='h',
...
var ='dens'
df_sorted = df_both.drop_duplicates(['date','Pressure [dbar]']).sort_values(['date','Pressure [dbar]'])
fig = df_sorted.plot.line(
    y="Pressure [dbar]", x=var, 
    facet_col='date', facet_col_wrap=9, 
...
fig.for_each_annotation(lambda a: a.update(text=a.text.split("=")[-1]))
if var=='temp': fig.update_xaxes(range=[-1,5], dtick=2)
fig.update_yaxes(
    range=[550,0]
     )
...
custom_scale  =[(0.0, "gold"), (0.94, "lemonchiffon"),  (0.97, "mediumseagreen"),  (0.989, "navy"), (1, "black")]
var ='temp'
other_var = 'sal'
...
for i in df_sorted.date.unique():
    diff_monthly = df_sorted[df_sorted.date == i].set_index('Pressure [dbar]')[530:550].reset_index()
    ...
    Y = diff_monthly["dens"].values.reshape(-1, 1)  # -1 means that calculate the dimension of rows, but have 1 column
...
df_monthly = pd.read_csv(f"{path_intermediate_files}/monthly_18_19_gf10.csv", index_col=0, parse_dates=["date"])
df_gf10 = df_monthly[(df_monthly['St.']=="GF10")&(df_monthly['Type']=='CTD')]
Z_1819 = df_gf10.drop_duplicates(["Pressure [dbar]", "date"], "first").pivot(index='Pressure [dbar]', columns='date', values='sal')
...
contourf = ax2.contourf(Z_1819.columns,Z_1819.index,Z_1819.values,levels=levels_f, norm=norm, cmap=cmap)
